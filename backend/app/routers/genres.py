...
def list_genres(request: Request, db: Session = Depends(get_db)) -> Envelope[list[GenreRead]]:
    ...
    return ok(
        [GenreRead.model_validate(g) for g in items],
...
def get_genre(
    ...
    return ok(
        GenreRead.model_validate(service.get_by_id(db, genre_id)),
        request_id=getattr(request.state, "request_id", None),
    )
...
def list_genre_movies(
    ...
    items, pagination = movie_svc.list(
        db,
        MovieFilters(genre_id=genre_id, page=page, page_size=page_size),
    )
    return ok(
